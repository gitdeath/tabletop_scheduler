...
| `LOG_LEVEL` | No | `info` | Logging verbosity. Options: `debug`, `info`, `warn`, `error`. Prise queries are logged only in `debug`. |
...
| Variable | Required | Default | Description |
|----------|:--------:|:-------:|-------------|
| `PUBLIC_URL` | No | - | Explicit public base URL (e.g., `https://scheduler.example.com`). Used for ensuring links generated in background jobs (Cron/Telegram) are correct. If not set, the app attempts to infer from request headers. |
